...
from reward_ensemble import EnsembleRewardPredictor
from transition_ensemble import EnsembleTransitionPredictor
...
def experiment(variant, bcq_policies, bcq_buffers, reward_ensemble_params_list, transition_ensemble_params_list, prev_exp_state=None):
    ...
    reward_ensemble_predictor = EnsembleRewardPredictor(reward_ensemble_params_list)

    transition_ensemble_predictor = EnsembleTransitionPredictor(transition_ensemble_params_list)
    ...
    trainer = SuperQTrainer(
        reward_ensemble_predictor=reward_ensemble_predictor,
        transition_ensemble_predictor=transition_ensemble_predictor,
        num_network_ensemble=variant['num_network_ensemble'],
        bcq_policies=bcq_policies,
        triplet_margin=variant['triplet_margin'],
        reward_std_threshold=variant['reward_std_threshold'],
        next_obs_std_threshold=variant['next_obs_std_threshold'],
...
if __name__ == "__main__":
    ...
    assert float(args.config[-3]) + float(args.config[-1]) / 10 == variant['triplet_margin']
    print('triplet_margin: ', float(args.config[-3]) + float(args.config[-1]) / 10)
    ...
    sub_reward_ensemble_params_dir = f"reward_prediction_ensemble/{domain}/mode_{exp_mode}/max_path_length_{max_path_length}/goal_"
    reward_ensemble_params_dir = osp.join(args.data_models_root, sub_reward_ensemble_params_dir)

    sub_transition_ensemble_params_dir = f"transition_prediction_ensemble/{domain}/mode_{exp_mode}/max_path_length_{max_path_length}/goal_"
    transition_ensemble_params_dir = osp.join(args.data_models_root, sub_transition_ensemble_params_dir)
    ...
    # Load reward_ensemble parameters
    reward_ensemble_params_list = []
    for idx in idx_list:
        params_dir = reward_ensemble_params_dir + str(idx) + '/itr_200.zip_pkl'
        params = load_gzip_pickle(params_dir)
        reward_ensemble_params_list.extend(params['trainer']['network_ensemble_state_dict'])

    # Load transition_ensemble parameters
    transition_ensemble_params_list = []
    for idx in idx_list:
        params_dir = transition_ensemble_params_dir + str(idx) + '/itr_200.zip_pkl'
        params = load_gzip_pickle(params_dir)
        transition_ensemble_params_list.extend(params['trainer']['network_ensemble_state_dict'])
    ...
    experiment(variant, bcq_policies, bcq_buffers, reward_ensemble_params_list, transition_ensemble_params_list, prev_exp_state=None)

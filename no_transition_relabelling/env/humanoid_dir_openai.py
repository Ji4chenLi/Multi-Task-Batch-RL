...
class HumanoidDirEnvOpenAI(HumanoidEnv):

    def __init__(self, goal=None):
        if goal is None:
            goal = 1.0
        self._goal = goal
        super(HumanoidDirEnvOpenAI, self).__init__()
    ...
    def step(self, action):
        ...
        lin_vel_cost = 1.25 * np.sum(goal_direction * (pos_after - pos_before)) / self.model.opt.timestep

...
class ReplayBuffer:
    ...
    def load_from_gzip(self, filename):
        def load_gzip_pickle(path, mode='rb'):
            import pickle
            import gzip
        ...
        self.obs2_buf = np.stack(list(data[:, 1])).astype(np.float32)
        self.acts_buf = np.stack(list(data[:, 2])).astype(np.float32)
        self.rews_buf = data[:, 3].astype(np.float32)

...
class SuperQTrainer(object):
    ...
    # @profile
    def train(self, batch, batch_idxes, train_goals):
        ...
        """
        with torch.no_grad():
            contexts_next_obs = contexts[:, obs_dim + action_dim : 2 * obs_dim + action_dim]
            
            relabeled_rewards = calculate_rewards(contexts_next_obs, train_goals)
            gt.stamp('relabel_rewards', unique=False)

            relabeled_rewards_mean = relabeled_rewards
            relabeled_rewards_std = torch.zeros_like(relabeled_rewards_mean)

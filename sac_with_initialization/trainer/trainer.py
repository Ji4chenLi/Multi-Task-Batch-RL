...
class SACTrainer(object):
    ...
    def train_from_torch_qf1(self, batch):
        ...
        with torch.no_grad():
            inferred_mdp = torch.repeat_interleave(self._inferred_mdp, obs.shape[0], dim=0)

        """
        QF Loss
        """
        q1_pred = self.qf1(obs, actions, inferred_mdp)
        ...
        target_q1_values = torch.min(
            self.target_qf1(next_obs, new_next_actions, inferred_mdp),
            self.target_qf2(next_obs, new_next_actions, inferred_mdp),
        ) - alpha * new_log_pi
    ...
    def train_from_torch_qf2_policy(self, batch):
        ...
        q2_pred = self.qf2(obs, actions, inferred_mdp)
        # Make sure policy accounts for squashing
        # functions like tanh correctly!
        new_next_actions, _, _, new_log_pi, *_ = self.policy(
            next_obs, reparameterize=True, return_log_prob=True,
        ...
        q_new_actions = torch.min(
            self.qf1(obs, new_obs_actions, inferred_mdp),
            self.qf2(obs, new_obs_actions, inferred_mdp),
        )
    ...
    def update_inferred_mdp_target_policy(self, inferred_mdp):
        self.target_policy.load_state_dict(self.policy.state_dict())

        self._inferred_mdp = inferred_mdp

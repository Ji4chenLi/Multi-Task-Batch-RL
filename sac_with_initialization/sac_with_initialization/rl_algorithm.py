...
class BatchRLAlgorithm(metaclass=abc.ABCMeta):
    def __init__(
            ...
            tiMe_data_collector: tiMeSampler,
        ...
        self.tiMe_data_collector = tiMe_data_collector
    ...
    def _log_stats(self, epoch):
        ...
        logger.record_dict(
            self.check_q_funct_estimate(remote_eval_paths),
    ...
    def check_q_funct_estimate(self, paths):
        ...
        inferred_mdps = torch.repeat_interleave(self.trainer._inferred_mdp, s0.shape[0], dim=0) 
        ...
        for path in paths:
            ...
            discount_cof = self.trainer.discount ** np.arange(len(path["rewards"]))

    ...
    def save_itr_params(self, itr, params):
        if self._snapshot_dir:
            if self._snapshot_mode == 'all':
                ...
                # pickle.dump(params, gzip.open(file_name, "wb"))
                file_name = osp.join(self._snapshot_dir, 'replay_buffer_' + str(self.goal) + '.zip_pkl')
                pickle.dump(params['replay_buffer'], gzip.open(file_name, "wb"))
            ...
            # Save the params every snapshot_gap and override previously saved result
                ...
                file_name = osp.join(self._snapshot_dir, 'params.zip_pkl')
                pickle.dump(params, gzip.open(file_name, "wb"))
            elif self._snapshot_mode == "gap_and_final":
                if itr % self._snapshot_gap == 0:
                    file_name = osp.join(
                        self._snapshot_dir, 'itr_%d.zip_pkl' % itr)
                    pickle.dump(params, gzip.open(file_name, "wb"))

                if itr == self.num_epochs:
                    file_name = osp.join(self._snapshot_dir, 'params.zip_pkl')
                    pickle.dump(params, gzip.open(file_name, "wb"))
